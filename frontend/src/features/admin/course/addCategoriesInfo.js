...
const AddCategoriesInfo = () => {
  ...
  const lastCategory =
    categoryData.length > 0
      ? categoryData[categoryData.length - 1]?.words.length < 10
  ...
  const categorySchema = z.object({
    name: z
      ...
      .refine(
        (val) => {
          if (val === lastCategory?.name) return true
  ...
  const onSubmit = (data) => {
    ...
    if (data.words.length < 10) {
      const addWord = window.confirm(
        "you can step to the next steps if you have at least 10 words in the category\n whould u like to move to add words"
      )
      if (addWord) dispatch(goToStep(1))
      return
    }
    const addAnother = window.confirm("whould you like to add another category???")
    reset({ words: [], name: "" })
    setSumSelected(0)
    if (!addAnother) dispatch(goToStep(3))
  }
        ...
        >
          ...
          <div>
            <label style={{ fontWeight: "bold" }}>
              Select Words <span>{sumSelected}</span>
            </label>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 6 }}>
              {(() => {
                // מילים שלא שויכו לקטגוריה אחרת
                const notUse = wordData.filter(
                  (word) => !word.categoryName || word.categoryName === lastCategory?.name
                )

                return notUse.length > 0 ? (
                  notUse.map((word, i) => {
