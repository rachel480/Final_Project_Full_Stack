...
const OptionsReview = ({ question, isKidFriendly }) => {
  ...
  return (
    <div className="flex flex-wrap gap-4 max-md:gap-2 mt-4 justify-center">
      {options.map((option, i) => {
        ...
        return (
          ...
          >
            {src ? (
              <img src={src} alt={option?.word} className="w-20 h-20 max-md:w-14 max-md:h-14 object-contain rounded-lg shadow-md" />
            ) : (
              <span className="text-sm max-md:text-xs font-bold text-indigo-700">{option?.word}</span>
            )}
            {!src && (
              <span className="mt-1 text-indigo-700 font-bold text-sm max-md:text-xs">
                {option?.word}
              </span>
            )}
          </button>
